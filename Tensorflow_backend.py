...
import numpy as np
class NN:
    def __init__Pre(self,batchsize,channelsize,width,height):
        ...
        self.action_index = tf.expand_dims(self.action_index,0)
        action_index = tf.concat([self.action_index,self.action_index,self.action_index,self.action_index,self.action_index],axis = 1)
        print(self.y,action_index)
    ...
    def __init__(self,batchsize,channelsize,width,height):
        ...
        self.qvalue_action = tf.gather_nd(self.act,action_index)
    ...
    def cal_loss(self,np_images_previous,np_image_after,reward,action_index,gamma):
        _,l = self.sess.run([self.train_op,self.loss],feed_dict={self.inputs:np_images_previous,self.inputs_next_step:np_image_after,self.r:reward,self.gamma:gamma,self.action_index:action_index})
        print(l)
